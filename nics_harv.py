...
def generate_cubefile(geom, grid, grid_values, dx, dy, dz, nptx, npty, nptz):
   ...
   fio.write("head 1\n".format())
   fio.write("head 2\n".format())
   fio.write("{0:5d} {1[0]:12.6f} {1[1]:12.6f} {1[2]:12.6f}\n".format(nat,grid[0]/Bohr2Angstrom))
   fio.write("{:5d} {:12.6f} {:12.6f} {:12.6f}\n".format(nptx, dx/Bohr2Angstrom,0,0))
   fio.write("{:5d} {:12.6f} {:12.6f} {:12.6f}\n".format(npty, 0,dy/Bohr2Angstrom,0))
   fio.write("{:5d} {:12.6f} {:12.6f} {:12.6f}\n".format(nptz, 0,0,dz/Bohr2Angstrom))
   for el in geom:
...
def generate_values_on_grid(geom, nics_grid):
    ...
    for x in np.linspace(xmin, xmax, npts):         # see dz
        for y in np.linspace(ymin, ymax, npts):     # see dz
            for z in np.linspace(zmin, zmax, npts): # dz = ( zmax - zmin ) / ( npts - 1 )
                tmp_lst.append([x,y,z])
    grid = np.array(tmp_lst)
    grid_values = {}
    dx = (xmax - xmin) / ( nptx - 1 ) # remember dz = ( zmax - zmin ) / ( npts - 1 )
...
#
  ...
  grid, grid_values, dx, dy, dz, nptx, npty, nptz = generate_values_on_grid(geom, nics_grid)
